# E4T5 — Implement /settings command

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

Add /settings command to configure restaurant parameters. Write tests to tests/specs/E4T5.json and append to tests/commands.json.
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E4T5.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E4T5.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
