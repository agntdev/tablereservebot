# fix-799f862b3f395bf8 — Fix: Failing dialog spec: typed party size is accepted after type-a-number prompt
...
The Tests-gate replay harness reported spec "typed party size is accepted after type-a-number prompt" as failing. 2 spec(s) failed: [typed party size is accepted after type-a-number prompt /reschedule reports unavailable when persistent storage is not configured]

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-799f862b3f395bf8.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-799f862b3f395bf8.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
