# fix-63a52de945bb6bd4 — Booking confirmation lacks guest-facing Reschedule/Cancel inline buttons

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

The spec (`docs/spec.md` §4, Interaction Flow 1) states: "Bot creates booking, allocates specific tables (internal), sends confirmation message with concise reference code and **inline buttons: View / Reschedule / Cancel**."

However, `finalizeBooking` at `src/bot.ts:198-208` sends the confirmation with `reply_markup: mainMenu()` — a generic menu with "How to Reserve", "Need Assistance?", and "Settings". There are no per-booking inline buttons for View/Reschedule/Cancel. Guests must discover and use slash-commands (`/reschedule`, `/booking`) instead, which is not documented in the start/welcome flow.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-63a52de945bb6bd4.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-63a52de945bb6bd4.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
