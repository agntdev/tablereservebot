# fix-b6303a21a00158ba — No ownership verification: anyone with a ref code can reschedule any booking

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

The `/reschedule` handler (`bot.ts:648`) looks up the booking by ref code only — it does not verify that `ctx.from.id` matches the booking's `guest_telegram_id`. Anyone who knows or guesses a valid ref code can reschedule (and effectively hijack) another guest's booking.

This may be intentional for restaurant-style bookings, but the task spec does not discuss access control. If this is unintended, add a `booking.guest_telegram_id !== ctx.from.id` check after line 648.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-b6303a21a00158ba.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-b6303a21a00158ba.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
