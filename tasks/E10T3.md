# E10T3 — Update /start message instructions to explain button roles

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

Add a short explanatory sentence in the /start message that explicitly states: 'Use the buttons below to reserve, get help, or configure restaurant settings (for owners only)' to establish clear expectations.
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E10T3.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E10T3.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
