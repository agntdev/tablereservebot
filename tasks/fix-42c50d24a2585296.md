# fix-42c50d24a2585296 — Reminder timing uses UTC instead of restaurant timezone

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

In `src/reminder.ts:43-50`, booking timestamps are constructed with `Date.UTC()`:
```ts
...
```
This treats the booking time's `HH:MM` as a **UTC time**, but the booking time is in restaurant local time. The resulting `bookingTs` is then compared against `now.getTime()` (server system time). For a restaurant in UTC+8 with a booking at 09:00, `Date.UTC` produces a timestamp 8 hours early, causing reminders to fire at the wrong time (or never fire).

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-42c50d24a2585296.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-42c50d24a2585296.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
