# fix-0a4b117c2835eee7 — Calendar date-picker allows booking past dates

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

The `/book` command validates and rejects past dates (`src/bot.ts:567`), but the calendar-based booking flow does not. The `cal:pick` callback handler (`src/bot.ts:419-428`), `showAvailableSlots` (`src/bot.ts:76`), and `listBookableSlots` (`src/availability.ts:10`) never check whether the selected date is in the past. The calendar grid (`src/calendar.ts`) renders the current month without disabling past dates. This allows users to select and potentially complete bookings for dates that have already passed, bypassing the `/book` command's validation.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-0a4b117c2835eee7.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-0a4b117c2835eee7.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
