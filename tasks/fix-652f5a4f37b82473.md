# fix-652f5a4f37b82473 — Test coverage incomplete: successful reschedule flow is untested

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 TBLBOT

The test spec `tests/specs/E2T1.json` only covers two scenarios: (1) `/reschedule` with no arguments shows usage, and (2) `/reschedule REF-ABC123` with no storage shows an error. No test exercises the primary happy path: a successful end-to-end reschedule where a booking is found, released, a new slot is picked, and the confirmation message is emitted. The gate would pass without catching the rollback and zombie-allocation bugs above.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-652f5a4f37b82473.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-652f5a4f37b82473.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
